...
RUN pip3 install --upgrade pip && mkdir psa-car-controller-fix && pip3 install poetry
COPY ./dist/psa_car_controller_fix-${PSACC_VERSION}-py3-none-any.whl .
RUN pip3 install --no-cache-dir psa_car_controller_fix-${PSACC_VERSION}-py3-none-any.whl
EXPOSE 5000
